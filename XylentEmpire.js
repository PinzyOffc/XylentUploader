...
: m.key.participant || m.key.remoteJid;
        ...
        const _mc = global._mc || (global._mc = new Map());
...
async function showProgress(chat, sentKey, steps = 10, delayMs = 500) {
    for (let i = 1; i <= steps; i++) {
        const bar = '▓'.repeat(i) + '░'.repeat(steps - i);
        const text = `⌛ Sedang memproses...\n[${bar}] ${i * 10}%`;
        try {
            await xylent.sendMessage(chat, { text, edit: sentKey });
        } catch (err) {
    ...
    await xylent.sendMessage(chat, { text: '🎉 Selesai Kak!', edit: sentKey });
}
...
if (!xylent.public && !isCreator && !global.owneroff) return;
...
'vcard': `BEGIN:VCARD\nVERSION:3.0\nN:XL;ttname,;;;\nFN:ttname\nitem1.TEL;waid=6285183387097:6285183387097\nitem1.X-ABLabel:Ponsel\nEND:VCARD`,
sendEphemeral: true
}}
}

if (global.maintenance && !isCreator) {
    const sudahDapet = _mc.has(m.sender);    
    if (!sudahDapet) {
        _mc.set(m.sender, Date.now());
        await xylent.sendMessage(m.chat, {
            text: `( 👋 ) Hallo Kak @${m.sender.split('@')[0]} Maaf Ya Sistem Kami Sedang Dalam *Maintenance Mode*.\nSilahkan Tunggu Beberapa Saat, Bot Akan Segera Kembali Online! 🙏\nJika Kakak Mau Request Fitur Di Bot Bisa Gunakan *reqfitur* Otomatis Bot Akan Kirim Pesan Ke Owner!.`,
            contextInfo: {
                mentionedJid: [m.sender],
                externalAdReply: {
                    showAdAttribution: true,
            ...
            viewOnceMessage: {
                message: {
                    interactiveMessage: proto.Message.InteractiveMessage.create({
                        ...
                        footer: proto.Message.InteractiveMessage.Footer.create({
                            text: `[ ! ] 𝐎𝐥𝐚 \`${pushname}\` 時間 ${getGreeting(parseInt(nowJakarta.format('HH')))}\n*🌙✨ SELAMAT HARI RAYA IDUL ADHA 1447 H ✨🌙* Tebarkan Kebaikan, Perkuat Keikhlasan, Dan Bahagiakan Sesama.。
╭────────────────────
│─▢ \`「 𝐈𝐧𝐟𝐨𝐫𝐦𝐚𝐭𝐢𝐨𝐧 𝐁𝐨𝐭 」\`
│─▢  Name Bot : Xylent MD
│─▢  Version : 💫 Special Idhul Adha
│─▢  Owner : Pinnxzy.t.me
...
case "send1xlihat": {
    if (!m.quoted) return reply("Reply foto/video yang mau dikirim sebagai viewOnce!")
    let nomor = args[0]
    if (!nomor) return reply(`Contoh:\n${prefix}send1xlihat 628xxxx`)
    ...
    try {
        ...
        let mimetype = quotedMsg.mimetype || ""
        let mediaType = mimetype.includes("video") ? "video" :
                        mimetype.includes("audio") ? "audio" : "image"

        let media = await downloadContentFromMessage(quotedMsg, mediaType)
        ...
        if (mediaType === "video") {
            await xylent.sendMessage(jid, {
                ...
                viewOnce: true  // ← ini yang bikin jadi 1x lihat
            })
        ...
        reply("✅ Berhasil dikirim sebagai viewOnce!")
...
case 'nulis': {
    if (!text) return m.reply('Mau nulis apa? Contoh:\n.nulis aku sayang kamu');

    await reaction(m.chat, "⏳");
    const progressMsg = await xylent.sendMessage(m.chat, { text: '⌛ Sedang memproses...\n[░░░░░░░░░░] 0%' }, { quoted: m });
    await showProgress(m.chat, progressMsg.key, 10, 400);
    ...
    try {
        ...
        const totalLines = Math.max(20, lines.length + 5);
        const canvasHeight = paddingTop + totalLines * lineHeight + 40;
        ...
        ctx.fillText('Hari :', canvasWidth - 200, 35);

        // Garis titik-titik header
        ctx.strokeStyle = '#aaaaaa';
        ctx.setLineDash([2, 3]);
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(canvasWidth - 155, 35);
        ctx.lineTo(canvasWidth - 30, 35);
        ctx.stroke();
        ctx.setLineDash([]);

        // Garis tebal atas (bawah header)
        ctx.strokeStyle = '#888888';
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.moveTo(30, 50);
        ctx.lineTo(canvasWidth - 20, 50);
        ctx.stroke();

        // Gambar semua baris + kotak checkbox kiri
        for (let i = 0; i < totalLines; i++) {
            ...
            ctx.beginPath();
            ctx.moveTo(30, y + 6);
            ctx.lineTo(canvasWidth - 20, y + 6);
            ctx.stroke();
        }

        // Tulis teks - efek tinta bolpen hitam
        ...
        await reaction(m.chat, "❌");
        return m.reply('❌ Error pas generate gambar, cek apakah canvas sudah terinstall.');
    }
}
break;
//================================================================================
...
case 'rvo': {
    ...
    try {
        ...
        const detailCaption = 'Nih Bang Isi Nya';
        const sendOptions = { quoted: m };

        if (kind === 'image') {
            await xylent.sendMessage(m.chat, { image: buffer, caption: detailCaption }, sendOptions);
        } else if (kind === 'video') {
            await xylent.sendMessage(m.chat, { video: buffer, caption: detailCaption }, sendOptions);
        } else if (kind === 'audio') {
            await xylent.sendMessage(m.chat, { audio: buffer, mimetype: 'audio/mpeg', ptt: true }, sendOptions);
        }

        await reaction(m.chat, "✅");
        return m.reply(`Type : *${kind.toUpperCase()}*\nViewOnce Berhasil Diproses.`);
    } catch (err) {
        console.error('rvo error:', err);
        await reaction(m.chat, "❌");
        return m.reply('❌ Gagal memproses media. Coba lagi atau kirim ulang.');
    }
}
break;
...
case "promote": {
    if (!isGroup) return ReplyXylent(mess.group)
    if (!isAdmins && !isCreator) return ReplyXylent('❌ Kamu bukan admin atau owner!')
    ...
    try {
        ...
        ReplyXylent(`✅ Berhasil mempromote @${target.split('@')[0]}`, { mentions: [target] })
    } catch (e) {
        console.error(e)
        ReplyXylent("❌ Gagal melakukan promote, pastikan bot adalah admin grup!")
    }
}
break
...
case "demote": {
    ...
    try {
        ...
        ReplyXylent(`✅ Berhasil mendemote @${target.split('@')[0]}`, { mentions: [target] })
    } catch (e) {
        console.error(e)
        ReplyXylent("❌ Gagal melakukan demote, pastikan bot adalah admin grup!")
    }
}
break
...
case "dor": {
    ...
    try {
        ...
        console.log(err)
        ReplyXylent("❌ Gagal kick user, pastikan bot adalah admin grup!")
    }
}
break
...
case 'warning': {
    ...
    let target = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : false
    if (!target) return ReplyXylent('❌ Tag atau reply pesan user yang mau diwarn')
    if (target === botNumber) return ReplyXylent('❌ Ga bisa warn bot sendiri')
    if (groupAdmins.includes(target)) return ReplyXylent('❌ Ga bisa warn sesama admin')

    const warnPath = './database/warn.json'
    let warnDb = {}
    if (fs.existsSync(warnPath)) {
        warnDb = JSON.parse(fs.readFileSync(warnPath))
    }

    if (!warnDb[m.chat]) warnDb[m.chat] = {}
    if (!warnDb[m.chat][target]) warnDb[m.chat][target] = 0

    warnDb[m.chat][target]++
    const warnCount = warnDb[m.chat][target]
    const maxWarn = 3

    fs.writeFileSync(warnPath, JSON.stringify(warnDb, null, 2))

    if (warnCount >= maxWarn) {
        warnDb[m.chat][target] = 0
        fs.writeFileSync(warnPath, JSON.stringify(warnDb, null, 2))

        await xylent.sendMessage(m.chat, {
            text: `Orang Goblok Ini @${target.split('@')[0]} Telah Dikeluarkan Dari Grup!\n📌 *Alasan:* Melanggar Aturan Grup\n_Mohom Patuhi Aturan Grub Yang Sudah Berlaku._`,
            mentions: [target]
        }, { quoted: m })

        await xylent.groupParticipantsUpdate(m.chat, [target], 'remove')
    } else {
        await xylent.sendMessage(m.chat, {
            text: `( ⚠️ ) @${target.split('@')[0]} Mendapatkan Peringatan Dari Admin!\n• Peringatan: *${warnCount}/${maxWarn}*\n• Jika Mencapai ${maxWarn}x Peringatan, Kamu Akan Dikeluarkan Dari Grup!`,
            mentions: [target]
        }, { quoted: m })
    }
}
...
case 'hapuswarn': {
    ...
    let target = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : false
    if (!target) return ReplyXylent('❌ Tag atau reply pesan user yang mau direset warnnya')
    ...
    if (!warnDb[m.chat] || !warnDb[m.chat][target] || warnDb[m.chat][target] === 0) {
        return ReplyXylent(`❌ Orang Ini @${target.split('@')[0]} Tidak Memiliki Riwayat Peringatan`, { mentions: [target] })
    }

    warnDb[m.chat][target] = 0
    fs.writeFileSync(warnPath, JSON.stringify(warnDb, null, 2))

    ReplyXylent(`✅ Peringatan @${target.split('@')[0]} Berhasil Direset!`, { mentions: [target] })
}
break
